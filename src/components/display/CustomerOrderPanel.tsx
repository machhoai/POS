...
const CustomerOrderPanel: React.FC<CustomerOrderPanelProps> = ({ order, language }) => {
        ...
        <section className="flex h-full min-h-[320px] flex-col overflow-hidden rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[var(--shadow-md)]">
            <header className="flex items-center justify-between border-b border-[var(--color-border)] p-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-extrabold text-[var(--color-text-primary)]">{copy.orderTitle}</h2>
                        <p className="mt-0.5 text-sm text-[var(--color-text-muted)]">{copy.productCount(itemCount)}</p>
                    </div>
                </div>
            </header>
            {order.member ? (
                <div className="flex items-center gap-3 px-4 pt-2">
                    <p className="text-lg font-semibold text-emerald-800">{copy.member}</p>
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="truncate text-lg font-extrabold text-emerald-950">
                            {order.member.fullName || copy.memberFallback}
                        </p>
            ...
            <ul className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-6 py-3" aria-label={copy.itemList}>
            ...
            <footer className="bg-white rounded-xl px-3 py-3">
                <div className="flex items-center justify-between gap-4">
                    <span className="text-md font-bold text-right flex-1 tracking-[0.12em] text-[var(--color-text-muted)]">{copy.total}</span>
