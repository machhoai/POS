  ...
  return (
    <section className="rounded-3xl border border-amber-200 bg-white p-4 shadow-sm md:p-5">
      <div className="mb-4 flex items-start gap-3">
        ...
        <div><h2 className="text-lg font-extrabold">Nạp bù / trừ điểm thẻ</h2><p className="text-xs text-[var(--color-text-muted)]">Nhập số dương để cộng điểm, số âm để trừ điểm.</p></div>
      </div>
      <form onSubmit={(event) => { event.preventDefault(); onReview(); }} className="space-y-4">
        <label className="block text-sm font-bold">Số điểm điều chỉnh *
          <input type="number" inputMode="numeric" min={-10_000_000} max={10_000_000} step={1} value={draft.amount ?? ""} onChange={(event) => onChange({ amount: event.target.value === "" ? null : Number(event.target.value) })} disabled={busy} className="mt-1.5 h-13 w-full rounded-2xl border border-[var(--color-border-subtle)] px-4 text-lg font-extrabold focus:border-[var(--color-accent)] disabled:bg-neutral-50" placeholder="Ví dụ: 50 để cộng, -50 để trừ" />
        </label>
        ...
        <div className="flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="mt-0.5 size-5 shrink-0" /><p>Thao tác này làm thay đổi trực tiếp số dư của khách và được ghi vĩnh viễn vào nhật ký kiểm toán.</p></div>
        {failed ? <p className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{mutation.failureReason}</p> : null}
        <button type="submit" disabled={busy} className="flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-amber-600 px-5 font-extrabold text-white shadow-sm active:scale-[0.98] disabled:opacity-50">{busy ? <LoaderCircle className="size-5 animate-spin" /> : <WalletCards className="size-5" />}{busy ? "Đang xử lý" : draft.amount && draft.amount < 0 ? "Kiểm tra trừ điểm" : "Kiểm tra cộng điểm"}</button>
      </form>
    </section>
  );
}
