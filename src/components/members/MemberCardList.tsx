...
import type { MemberCard, RemoteRequestStatus } from "@/lib/types/member";
...
const MemberCardList: React.FC<MemberCardListProps> = ({ status, cards, error, onReload }) => {
  ...
  return <section className="min-h-72 rounded-3xl border border-[var(--color-border)] bg-white p-4 shadow-sm">
    <header className="mb-4 flex items-center justify-between gap-3"><div><h3 className="font-extrabold">Thẻ thành viên hiện tại</h3><p className="text-xs text-[var(--color-text-muted)]">OpenAPI chưa cung cấp lịch sử cấp đổi trong quá khứ.</p></div><button type="button" onClick={onReload} disabled={loading} className="flex min-h-11 items-center gap-2 rounded-xl border border-[var(--color-border)] px-4 text-sm font-bold disabled:opacity-50"><RefreshCw className={`size-4 ${loading ? "animate-spin" : ""}`} />Làm mới</button></header>
