...
import { saveRemoteReceiptSettings } from "@/lib/services/deviceEnrollmentService";
...
export default function ReceiptSettingsPage() {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            const synchronizedSettings = mapRemoteReceiptSettings(saved);
            applyRemoteSettings(saved.warehouse_id, saved.version, synchronizedSettings);
            setDraftSettings(null);
            showSuccess(
                "Đã đồng bộ cấu hình biên lai",
                "Thay đổi đã được lưu trên JPULSE và sẽ tự cập nhật đến các máy POS khác.",
            );
        } catch (error: unknown) {
            console.error("[Cấu hình biên lai] Không thể đồng bộ JPULSE:", error);
            showError(
                "Không thể lưu cấu hình biên lai",
